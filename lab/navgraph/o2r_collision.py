...
Layout is transcribed from Shipwright source (the authority on this format):
...
if __name__ == "__main__":
    ...
    m = load_from_o2r(o2r, entry)
    print(f"bounds {m.min_bounds} .. {m.max_bounds}")
    print(f"{len(m.vertices)} vertices, {len(m.polys)} polys, "
          f"{len(m.surface_types)} surface types, {len(m.water_boxes)} water boxes")
    floors = sum(1 for p in m.polys if p.ny > 0.6)
    walls = sum(1 for p in m.polys if abs(p.ny) <= 0.3)
    ceils = sum(1 for p in m.polys if p.ny < -0.6)
    print(f"classified: {floors} floor-ish, {walls} wall-ish, {ceils} ceiling-ish")
    climb = {}
    for p in m.polys:
        st = m.surface(p)
        if st.is_ladder or st.is_vine or st.is_crawlspace:
            kind = "ladder" if st.is_ladder else ("vine" if st.is_vine else "crawl")
            climb[kind] = climb.get(kind, 0) + 1
    print(f"climbable/crawl polys by kind: {climb}")
    ys = sorted(v[1] for v in m.vertices)
    print(f"vertex y range: {ys[0]} .. {ys[-1]}")
